...
void Game_scene::sdl_initialize_renderer(SDL_Window* window, const int& screen_width, const int& screen_height)
{
  renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED);
  if (renderer == nullptr)
  {
    throw std::runtime_error{"Game_scene::sdl_initialize_renderer(), renderer is NULL!"};
  }
  ...
  px = static_cast<float>(sw) * 0.5f;
  py = static_cast<float>(sh) * 0.5f;
  spawn_player();
}
...
void Game_scene::movement_system()
{
  ...
  if (player->input->up) player->transform->velocity.y -= 5.0f;
  if (player->input->left) player->transform->velocity.x -= 5.0f;
  if (player->input->right) player->transform->velocity.x += 5.0f;
  if (player->input->down) player->transform->velocity.y += 5.0f;
  // 대각 이동 정규화
       ...
       (player->input->down && player->input->right) ) 
  {
    ...
    player->transform->velocity.normalize().scale(5.0f);
  }
  // player 의 위치 업데이트
  ...
  if ((player->transform->position.x + player->shape->radius) >= static_cast<float>(sw))
  {
    player->transform->position.x = static_cast<float>(sw) - player->shape->radius;
  }
  // screen height
  ...
  if ((player->transform->position.y + player->shape->radius) >= static_cast<float>(sh))
  {
    player->transform->position.y = static_cast<float>(sh) - player->shape->radius;
  }
  ...
  {
    e->transform->velocity.normalize().scale(5.0f);
  ...
  for (auto& e : entities.get_entities("enemy"))
  {
    ...
    if ((e->transform->position.x + e->shape->radius) >= static_cast<float>(sw))
    {
      e->transform->position.x = static_cast<float>(sw) - e->shape->radius;
      e->transform->velocity.x *= -1.0f;
    }
    // screen height
    ...
    if ((e->transform->position.y + e->shape->radius) >= static_cast<float>(sh))
    {
      e->transform->position.y = static_cast<float>(sh) - e->shape->radius;
      e->transform->velocity.y *= -1.0f;
    }
  }
}
...
void Game_scene::liftspan_system()
{
  ...
  */
  ...
  for (auto& b : entities.get_entities())
  {
    ...
    b->lifespan->remaining = 0;
    // rgb 의 alpha 조정
    if (b->get_tag() == "bullet")  // bullet
    {
      b->shape->alpha -= 1;
    }
    else                           // 그 외
    ...
    // 시간 대신 alpha 사용
    if (b->shape->alpha <= 100) b->destroy();
  }
}
...
void Game_scene::render_system()
{
  ...
  {
    // render geometry
    SDL_SetRenderDrawColor(renderer, e->shape->fill_color_r, e->shape->fill_color_g, e->shape->fill_color_b, e->shape->alpha);
...
void Game_scene::spawn_player()
{
  ...
  */
  ...
  // 빨간색 thickness 만 사용
  entity->shape = std::make_shared<Shape_component>(32.0f, 8, 0xFF, 0x00, 0x00, 0xFF);

  // 플레이어에 입력 구성요소를 추가하여 입력을 사용할 수 있도록 합니다.
  entity->input = std::make_shared<Input_component>();
  entity->collision = std::make_shared<Collision_component>(32.0f);
...
void Game_scene::spawn_enemy()
{
  ...
  */
  ...
  float ex = static_cast<float>(rand() % sw);
  float ey = static_cast<float>(rand() % sh);
  

  // 날아가는 방향(direction)
  float dx = -static_cast<float>(sw) + static_cast<float>( (rand() % ( 1 + sw - (-sw) )) );
  float dy = -static_cast<float>(sh) + static_cast<float>( (rand() % ( 1 + sh - (-sh) )) );

  Vector2 dir = {dx, dy};
  int random_length = 2 + ( rand() % (1 + 7 - 2) );
  ...
  // 3 ~ 8 각형
  int random_sides = 3 + ( rand() % (1 + 8 - 3) );
  // 랜덤 색상
  Uint8 random_color1 = static_cast<Uint8>(0x00+(rand() % (1+0xFF-0x00)));
  Uint8 random_color2 = static_cast<Uint8>(0x00+(rand() % (1+0xFF-0x00)));
  Uint8 random_color3 = static_cast<Uint8>(0x00+(rand() % (1+0xFF-0x00)));
  // 랜덤 thickness 만 사용
  ...
  */
  ...
  for (int i = 0; i < entity->shape->sides; ++i)
  {
    ...
    small_enemies->shape = std::make_shared<Shape_component>(entity->shape->radius*0.5f, entity->shape->sides, 
                                                             entity->shape->fill_color_r, 
                                                             entity->shape->fill_color_g, 
                                                             entity->shape->fill_color_b, 0xFF);
    small_enemies->collision = std::make_shared<Collision_component>(entity->shape->radius*0.5f);
    small_enemies->lifespan = std::make_shared<Lifespan_component>(1);
  }
}
...
void Game_scene::spawn_bullet(std::shared_ptr<Entity> entity, const Vector2& target)
{
  ...
  bullet->transform = std::make_shared<Transform_component>(entity->transform->position, Vector2{target - entity->transform->position}, 0.0f);
  bullet->shape = std::make_shared<Shape_component>(10.0f, 8, 0xFF, 0xFF, 0xFF, 0xFF);
  bullet->collision = std::make_shared<Collision_component>(10.0f);
  bullet->lifespan = std::make_shared<Lifespan_component>(1);
}

void Game_scene::spawn_special_weapon(std::shared_ptr<Entity> entity)
{
